...
"""We implement the tuned HOG plus Linear SVM baseline for leakage-aware crop classification."""
...
def getDefaultTuningSearchSpace() -> dict[str, list[Any]]:
    """We define the default coarse HOG plus Linear SVM tuning space used for the classical baseline."""
    return {
        "imageSizeList": [96, 128],
        "cValueList": [0.5, 1.0, 2.0],
...
def normalizeTuningSearchSpace(searchSpace: dict[str, list[Any]]) -> dict[str, list[Any]]:
    ...
    if "colorModeList" not in normalizedSearchSpace:
        normalizedSearchSpace["colorModeList"] = ["grayscale"]
...
def fitLinearSvmModel(
    ...
    """We fit one Linear SVM trial and record whether convergence warnings occurred."""
...
def predictWithLinearSvmModel(
    modelPipeline: Pipeline,
    featureMatrix: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """We generate class predictions and decision scores for one feature matrix."""
    predictedClassIndices = modelPipeline.predict(featureMatrix)
    decisionScores = modelPipeline.decision_function(featureMatrix)

    if decisionScores.ndim == 1:
        decisionScores = decisionScores[:, None]

    return predictedClassIndices.astype(np.int64), decisionScores.astype(np.float32)
...
def buildPredictionTable(
    ...
    maxDecisionScores = decisionScores.max(axis=1)
    ...
    predictionDf["predictedDecisionScore"] = maxDecisionScores.astype(float)
...
def buildTuningSummaryDf(
    ...
    """We evaluate every allowed HOG plus Linear SVM configuration and summarize only train and validation performance."""
    tuningRows = []
    ...
    for cacheKey, cacheEntry in sortedCacheItems:
        ...
        for cValue in searchSpace["cValueList"]:
            ...
            modelPipeline, fitMeta = fitLinearSvmModel(
                trainFeatures=trainFeatures,
                trainTargets=trainTargets,
                seed=seed,
                cValue=float(cValue),
                tol=float(svmTol),
                maxIter=int(svmMaxIter),
            )

            trainPredictedIndices, _ = predictWithLinearSvmModel(
                modelPipeline=modelPipeline,
                featureMatrix=trainFeatures,
            )

            valPredictedIndices, _ = predictWithLinearSvmModel(
                modelPipeline=modelPipeline,
...
def saveBaselineArtifacts(
    ...
    modelOutputDir = projectConfig.outputsDir / "models" / "hogLinearSvm"
    tableOutputDir = projectConfig.tablesDir / "hogLinearSvm"

    modelOutputDir.mkdir(parents=True, exist_ok=True)
    tableOutputDir.mkdir(parents=True, exist_ok=True)

    modelPath = modelOutputDir / "hogLinearSvm.joblib"
    configPath = modelOutputDir / "hogLinearSvmConfig.json"
    tuningSummaryPath = tableOutputDir / "hogLinearSvmTuningSummary.csv"
    metricSummaryPath = tableOutputDir / "hogLinearSvmMetricSummary.csv"
    valPredictionPath = tableOutputDir / "hogLinearSvmValPredictions.csv"
    testPredictionPath = tableOutputDir / "hogLinearSvmTestPredictions.csv"
...
def runTunedHogLinearSvmBaseline(
    projectConfig: ProjectConfig,
    searchSpace: dict[str, list[Any]] | None = None,
    svmTol: float = 1e-3,
    svmMaxIter: int = 10000,
    verbose: bool = True,
) -> dict[str, Any]:
    """We tune the HOG plus Linear SVM baseline on validation macro F1 and save only the selected best artifacts."""
    ...
    bestModelPipeline, finalFitMeta = fitLinearSvmModel(
        trainFeatures=trainFeatures,
        trainTargets=trainTargets,
        seed=projectConfig.seed,
        cValue=bestCValue,
    ...
    trainPredictedIndices, trainDecisionScores = predictWithLinearSvmModel(
        modelPipeline=bestModelPipeline,
        featureMatrix=trainFeatures,
    )

    valPredictedIndices, valDecisionScores = predictWithLinearSvmModel(
        modelPipeline=bestModelPipeline,
        featureMatrix=valFeatures,
    )

    testPredictedIndices, testDecisionScores = predictWithLinearSvmModel(
        modelPipeline=bestModelPipeline,
    ...
    trainPredictionDf = buildPredictionTable(
        ...
        decisionScores=trainDecisionScores,
    ...
    valPredictionDf = buildPredictionTable(
        ...
        decisionScores=valDecisionScores,
        indexToClassMap=indexToClassMap,
    )
    ...
    testPredictionDf = buildPredictionTable(
        ...
        decisionScores=testDecisionScores,
        indexToClassMap=indexToClassMap,
    )
